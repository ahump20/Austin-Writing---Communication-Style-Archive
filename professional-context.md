# Professional Context — Austin Humphrey

Use this reference when writing in Austin's voice requires grounding in his actual professional background, credentials, or experience. This is factual material — do not embellish or infer beyond what's stated.

## Identity

Austin Humphrey. Based in Boerne, TX. Sports intelligence, product strategy, AI-assisted analytics.

Contact: ahump20@outlook.com · Austin@BlazeSportsIntel.com (BSI business) · (210) 273-5538
Web: AustinHumphrey.com · BlazeSportsIntel.com · linkedin.com/in/ahump20

## Experience

**Founder & Builder — Blaze Sports Intel** (Boerne, TX, 2023–Present)
Production sports intelligence platform covering MLB, NFL, NCAA football, NBA, and college baseball — entirely on Cloudflare infrastructure. Backend: 53 Workers, 12 D1 databases, 45 KV namespaces, 18 R2 buckets. Built real-time ingestion pipelines and mobile-first dashboards with ~30-second live refresh, AI-assisted forecasting modules, and the BlazeCraft reliability monitoring interface. Solo founder owning every layer: roadmap, architecture, release ops, content, stakeholder communication. Focused on markets that matter and don't get covered.

**Advertising Account Executive — Spectrum Reach** (Austin, TX, Nov 2022–Dec 2025)
Data-informed advertising strategies across linear TV, OTT/CTV, streaming, and digital for local and regional clients. Translated campaign performance into insights that drove client renewals and strategic upsells.

**Financial Representative — Northwestern Mutual** (San Antonio, TX, Dec 2020–Aug 2022)
Top-5 ranked intern program, converted to full-time advising role. "Power of 10" Award — top 10% nationally. Comprehensive financial plans through structured modeling, prospecting discipline, and consultative client relationships.

## What He's Built

- 53-Worker serverless architecture serving multi-sport analytics across five leagues — zero to production as a solo builder
- Two years designing Claude, Gemini, and ChatGPT workflows for sports data ingestion, forecasting, and editorial — deployed systems, not demos
- Performance metrics → editorial content for college baseball, FCS football, and other markets mainstream media won't touch
- Full-stack execution: architecture, frontend, content, ops, and stakeholder communication — every layer, one person

## Education

- **AI & Machine Learning — Postgraduate Certificate:** UT Austin McCombs School of Business (In Progress)
- **M.S. Entertainment Business — Sports Management:** Full Sail University (Feb 2026, GPA 3.56)
- **B.A. International Relations & Global Studies:** University of Texas at Austin (2014–2020), Minors: Economics, European Studies

## Technical Stack

- AI/LLM: Claude.ai, Claude Code, Gemini, ChatGPT, agentic workflow design (2 years hands-on)
- Cloud: Cloudflare Workers, D1, KV, R2, Pages
- Languages: TypeScript, Node.js, React, Next.js, Hono, SQL
- Data: REST APIs, WebSockets, real-time ingestion pipelines
- DevOps: GitHub, Wrangler CLI, CI/CD, Vitest

## Core Capabilities

Consultative sales, account strategy, client development, financial planning, multi-platform campaigns, API design, real-time pipelines, data visualization, AI/LLM workflows, agentic prompting, sports analytics, editorial strategy, product development.

## Leadership

- Rush Captain & Alumni Relations Chair — Alpha Tau Omega, UT Austin. Managed ~$100K event budget. Recruited 73 new members across two pledge classes.
- "Power of 10" Award — Northwestern Mutual. Top 10% nationally in intern program.

## Voice-Writing Implications

When writing professional content in Austin's voice, ground claims in these specifics rather than vague credentials. "53 Workers across five leagues" is stronger than "extensive infrastructure." "Top 10% nationally" is stronger than "high performer." The resume demonstrates the same voice principles: declarative confidence, evidence at point of need, no hype language, concrete over abstract.
